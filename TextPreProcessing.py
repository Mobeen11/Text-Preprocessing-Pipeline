...
class Preprocessing():
    ...
    def normalizeText(self, text, lowercase=True, removeNumberAndSpecialCharacter=True, removeDigits=False,
                            ...
                            removePuntuation=True, sentenceTokenizer=False, wordTokenizer=True, tokenLemmatizer=True, removeURLs=True, removeHTMLtags=True
                            ):
        try: 
            if lowercase:
            ...
            if wordTokenizer:
                if sentenceTokenizer:
                    raise Exception ("Word Tokenizer and Sentence Tokenizer can't be called at the same time")
